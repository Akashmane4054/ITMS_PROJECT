...
public class AdviceController extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleBussinessException(BussinessException e) {
        ...
        ErrorResponse error = new ErrorResponse(e.getErrorcode().toString(), e.getDescription(), e.getParameter(), BussinessException.class.getSimpleName());
    ...
    public ResponseEntity<Object> handleContractException(ContractException e) {
        log.error(DETAILED_EXCEPTION, e);
        ErrorResponse error = new ErrorResponse(e.getErrorcode().toString(), e.getDescription(), e.getParameter(), ContractException.class.getSimpleName());
        Map<String, Object> map = new HashMap<>();
        map.put(Constants.ERROR, error);
        map.put(Constants.SUCCESS, null);
        return new ResponseEntity<>(map, e.getErrorcode());
    ...
    public ResponseEntity<Object> handleTechnicalException(TechnicalException e) {
        log.error(DETAILED_EXCEPTION, e);
        ErrorResponse error = new ErrorResponse(e.getErrorcode().toString(), e.getDescription(), e.getParameter(), TechnicalException.class.getSimpleName());
        Map<String, Object> map = new HashMap<>();
        map.put(Constants.ERROR, error);
        map.put(Constants.SUCCESS, null);
        return new ResponseEntity<>(map, e.getErrorcode());
